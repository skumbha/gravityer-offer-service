...
class BuyOneGetOneV2Test {

    @Test
    public void testMaximizeDiscount_EmptyList() {
        List<Product> productList = new ArrayList<>();

        IBOGOOffer bogoOffer = new BuyOneGetOneV2();
        BOGOResponse response = bogoOffer.maximizeDiscount(productList);

        List<Product> expectedPayable = new ArrayList<>();
        List<Product> expectedDiscounted = new ArrayList<>();

        assertEquals(expectedPayable, response.getPayableProducts());
        assertEquals(expectedDiscounted, response.getDiscountedProducts());
    }

    @Test
    public void testMaximizeDiscount_SingleProduct() {
        List<Product> productList = Arrays.asList(new Product("Product1", 10));

        IBOGOOffer bogoOffer = new BuyOneGetOneV2();
        BOGOResponse response = bogoOffer.maximizeDiscount(productList);

        List<Product> expectedPayable = Arrays.asList(new Product("Product1", 10));
        List<Product> expectedDiscounted = new ArrayList<>();
    ...
    public void testMaximizeDiscountMultipleProductsV2() {
        ...
        productList.add(new Product("Item7", 60));
        productList.add(new Product("Item8", 60));
        productList.add(new Product("Item9", 60));
        ...
        assertEquals("Item7", response.getPayableProducts().get(0).getName());
        assertEquals("Item10", response.getPayableProducts().get(1).getName());
        assertEquals("Item9", response.getPayableProducts().get(2).getName());
        assertEquals("Item8", response.getPayableProducts().get(3).getName());
        assertEquals("Item2", response.getPayableProducts().get(4).getName());
        ...
        assertEquals("Item1", response.getDiscountedProducts().get(4).getName());
    }


    @Test
    public void testMaximizeDiscountMultipleProductsOddNumberOfProducts() {
        List<Product> productList = new ArrayList<>();
        productList.add(new Product("Item1", 10));
        productList.add(new Product("Item2", 20));
        productList.add(new Product("Item3", 30));
        ...
        assertEquals("Item7", response.getPayableProducts().get(0).getName());
        assertEquals("Item9", response.getPayableProducts().get(1).getName());
        assertEquals("Item8", response.getPayableProducts().get(2).getName());
        assertEquals("Item3", response.getPayableProducts().get(3).getName());
        assertEquals("Item1", response.getPayableProducts().get(4).getName());

        assertEquals("Item6", response.getDiscountedProducts().get(0).getName());
        assertEquals("Item4", response.getDiscountedProducts().get(1).getName());
        assertEquals("Item5", response.getDiscountedProducts().get(2).getName());
        assertEquals("Item2", response.getDiscountedProducts().get(3).getName());
